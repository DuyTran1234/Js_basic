...
function menu3()
{
	...
	var check = false;
	var delContact = readLineSync.question('Enter name must eidt in Contacs: ');
	for(var i = 0; i < nArray.length; i++)
	{
		if(delContact == nArray[i].name)
		{
			check = true;
			nArray[i].name = readLineSync.question('Edit name: ');
			nArray[i].phoneNumber = readLineSync.question('Edit phone number: ');
			break;
		}
	}
	if(check === false)
